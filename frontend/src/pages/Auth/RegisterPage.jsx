 ...
 * Handles account creation with validation and error feedback.
 */

import React, { useState, useEffect } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { Wallet, User, Mail, Lock, ArrowRight, Loader2, AlertCircle } from 'lucide-react';
...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await register(name, email, password, 'INR');
      showSuccess('Account created successfully! Welcome to Pocket C.A.');
      navigate(ROUTES.DASHBOARD || '/');
    } catch (err) {
      const msg = err?.response?.data?.error?.message || err?.message || 'Failed to create account. Please try again.';
      setError(msg);
      showError(msg);
    } finally {
  ...
  return (
    ...
    >
      ...
      <div
        className="fixed top-1/4 right-1/4 w-96 h-96 rounded-full opacity-10 blur-3xl pointer-events-none"
        style={{ background: 'var(--color-secondary)' }}
      />
      <div
        className="fixed bottom-1/4 left-1/4 w-96 h-96 rounded-full opacity-10 blur-3xl pointer-events-none"
        ...
        <div className="glass-card p-8">
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-sm mt-1" style={{ color: 'var(--color-text-secondary)' }}>
              Your AI-powered finance journey starts here
            </p>
          </div>
